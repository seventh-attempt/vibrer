...
class SongFactory(factory.django.DjangoModelFactory):
    title = factory.LazyAttribute(
        lambda x: faker.pystr(min_chars=5, max_chars=15))
...
class AlbumFactory(factory.django.DjangoModelFactory):
    title = factory.LazyAttribute(
        lambda x: faker.pystr(min_chars=5, max_chars=15))
    songs_amount = 0
    release_year = factory.LazyAttribute(
        lambda x: faker.date_between(start_date='-30y', end_date='now'))
    photo = factory.LazyAttribute(lambda x: faker.file_name(category="image"))
...
def fill_with_data(model: Union[Album, Artist, Genre, Song], min_limit: int,
                   max_limit: int) -> frozenset:
    """
    This function generates a collection with random size.
        Takes:
            model: queryset with multiple instances of one type
                                                (Album, Artist, Genre or Song).
